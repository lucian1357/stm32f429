...
/* ---------------- Calculator logic (display-agnostic) ---------------- */

static void Calc_Reset(CalculatorState *state) {
...
static bool Calc_InputHasDecimal(const CalculatorState *state) {
    return strchr(state->input, '.') != NULL;
}

static void Calc_ClearInput(CalculatorState *state) {
    memset(state->input, 0, sizeof(state->input));
}

static void Calc_AppendInput(CalculatorState *state, char value) {
    ...
    if (value == '.' && Calc_InputHasDecimal(state)) {
        return;
    }
...
static double Calc_ParseInput(const CalculatorState *state) {
    if (state->input[0] == '\0') {
        return 0.0;
    }
...
static double Calc_ApplyOperation(double a, double b, char op, bool *error) {
    switch (op) {
        case '+':
            return a + b;
        case '-':
...
static void Calc_FormatLine(const char *prefix, double value, char *output, size_t size) {
    char number[32];
...
static void Calc_PressDigit(CalculatorState *state, char digit) {
    Calc_AppendInput(state, digit);
}

static void Calc_PressDot(CalculatorState *state) {
    Calc_AppendInput(state, '.');
}

static void Calc_PressOperator(CalculatorState *state, char operatorChar) {
    double value = Calc_ParseInput(state);

    if (!state->hasAccumulator) {
        state->accumulator = value;
        state->hasAccumulator = true;
    } else if (state->currentOperator != 0 || state->input[0] != '\0') {
        state->accumulator = Calc_ApplyOperation(state->accumulator, value, state->currentOperator, &state->error);
    }

    Calc_ClearInput(state);
    state->currentOperator = operatorChar;
}

static void Calc_PressEquals(CalculatorState *state) {
    if (!state->hasAccumulator && state->input[0] != '\0') {
        state->accumulator = Calc_ParseInput(state);
        state->hasAccumulator = true;
        state->currentOperator = 0;
        Calc_ClearInput(state);
        return;
...
static void Calc_PressClear(CalculatorState *state) {
    Calc_Reset(state);
}

/* ---------------- UI helpers (rendering only) ---------------- */
...
static uint16_t UI_ButtonColor(const CalculatorButton *button) {
    switch (button->type) {
...
static void UI_DrawDisplay(const CalculatorState *state, const TouchStatus *touch) {
    char line1[64];
    char line2[64];
    char line3[64];
    ...
    if (state->error) {
        ...
        Calc_FormatLine("Result: ", state->accumulator, line1, sizeof(line1));
    } else {
...
static void UI_DrawKeypadBackground(void) {
...
static void UI_DrawButton(const CalculatorButton *button, bool pressed) {
    uint16_t background = UI_ButtonColor(button);
...
static void UI_DrawAllButtons(void) {
    ...
    UI_DrawKeypadBackground();

    for (i = 0; i < (sizeof(buttons) / sizeof(buttons[0])); i++) {
        UI_DrawButton(&buttons[i], false);
    }
}

/* ---------------- Touch/controller glue ---------------- */

static int8_t Controller_FindButton(uint16_t x, uint16_t y) {
    uint32_t i;
...
static void Controller_ProcessButton(CalculatorState *state, const CalculatorButton *button) {
    ...
    switch (button->type) {
        case CALC_BUTTON_DIGIT:
            if (button->label[0] == '.') {
                ...
                Calc_PressDigit(state, button->label[0]);
            }
            break;
        case CALC_BUTTON_OPERATOR:
            Calc_PressOperator(state, button->label[0]);
            break;
        case CALC_BUTTON_CLEAR:
            Calc_PressClear(state);
            break;
        case CALC_BUTTON_EQUALS:
            Calc_PressEquals(state);
            break;
        default:
            break;
    }
...
static void Controller_HandleTouch(CalculatorState *state, TouchStatus *touchStatus, int8_t *lastButton, bool *stateDirty) {
    ...
    if (TM_STMPE811_ReadTouch(&touchData) == TM_STMPE811_State_Pressed) {
        ...
        pressed = Controller_FindButton(touchData.x, touchData.y);

        if (pressed >= 0 && pressed != *lastButton) {
            Controller_ProcessButton(state, &buttons[pressed]);
            *stateDirty = true;
            UI_DrawButton(&buttons[pressed], true);
            *lastButton = pressed;
        }
    } else {
        touchStatus->x = 0;
        ...
        if (*lastButton >= 0) {
            UI_DrawButton(&buttons[*lastButton], false);
            *lastButton = -1;
        }
    }
}
...
int main(void) {
    ...
    UI_DrawDisplay(&state, &touchStatus);
    UI_DrawAllButtons();

    while (1) {
        Controller_HandleTouch(&state, &touchStatus, &lastButton, &stateDirty);
